...
export const changeApproval = createAsyncThunk(
  ...
  async ({ address, bond, provider, networkID }: IApproveBondAsyncThunk, { dispatch }) => {
    ...
    const treasuryAddr = addresses[networkID].PhantomTreasury;

    let approveTx;
    let allowance = await reserveContract.allowance(address, treasuryAddr);

    // return early if approval already exists
    if (allowance.gt(BigNumber.from("0"))) {
      dispatch(info("Approval completed."));
      // This dispatch needs to be called to update the approval, but user bonds require a nonce. Not sure how to address yet
  ...
  async ({ bond, bondType, value, provider, networkID }: ICalcBondDetailsAsyncThunk, { dispatch }): Promise<IBondDetails> => {
    ...
    try {
      ...
      if (value === 0) {
        ...
        valuation = await bondHelper.valuation(bond.reserveAddrs[networkID], value);
        bondQuote = BigNumber.from(valuation * bondDiscount);
      }
    } catch (e) {
      console.log("Error fetching data for bondQuote calculation", e);
    }

    try {
      [isAvailable, vestingTerm] = await bondHelper.infoOfBondType(bondType);
    } catch (e) {
      console.log("Error getting bond info", e);
    }
...
export const bondAsset = createAsyncThunk(
  ...
  async ({ value, address, bond, bondType, networkID, provider, slippage }: IBondAssetAsyncThunk, { dispatch }) => {
    ...
    try {
      bondTx = await bondHelper.bond(value, bond.reserveAddrs[networkID], bondType);
      dispatch(
